...
int main(int argc, char * argv[])
{
    ...
    while (!ans.seekEof() && !ouf.seekEof())
    {
        ...
        if (ja != YES && ja != NO)
            quitf(_fail, "%s or %s expected in answer, but %s found [%d%s case]",
        ...
        if (pa == YES)
            ...
            quitf(_pe, "%s or %s expected, but %s found [%d%s case]",
                  YES.c_str(), NO.c_str(), compress(pa).c_str(), index, englishEnding(index).c_str());

        if (ja != pa)
            quitf(_wa, "expected %s, found %s [%d%s case]",
    ...
    if (index == 0)
        ...
        quitf(_ok, "%d cases: yes count %d, no count %d. Well done! ", index, yesCount, noCount);
